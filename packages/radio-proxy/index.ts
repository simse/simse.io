...
Bun.serve({
  async fetch(req, server) {
    ...
    if (url.pathname === '/stream') {
      try {
        const audioResponse = await fetch('https://80.streeemer.com/listen/80s/radio.mp3');

        if (!audioResponse.ok) {
          return new Response(`Failed to fetch audio: ${audioResponse.status} ${audioResponse.statusText}`, { status: audioResponse.status });
        }
  ...
  websocket: {
    open(ws) {
      ...
      intervalMap.set(ws, setInterval(() => {
        // only send updates
